      ...
      <div class="flex items-end justify-between mb-6 flex-wrap gap-4">
        <div>
          ...
          <p class="text-sm text-muted mt-1 tabular-nums">
            {{ teams().length }} équipe<span>{{ teams().length > 1 ? 's' : '' }}</span> dans la
            communauté
          </p>
        </div>
        @if (auth.isPlayer()) {
          ...
          @for (team of teams(); track team.id) {
            ...
            >
              <div class="flex items-start justify-between mb-4">
                ...
                <span
                  class="tag-mono text-electric bg-electric/10 border border-electric/30 px-2 py-0.5 rounded shrink-0"
                >
                  {{ team.tag }}
                </span>
              </div>
              ...
              <div class="pt-3 border-t border-border flex items-center justify-between">
                <span class="text-xs text-muted tabular-nums">
                  {{ team.members?.length ?? 0 }} membre<span>{{
                    (team.members?.length ?? 0) > 1 ? 's' : ''
                  }}</span>
                </span>
                <span class="text-xs text-muted group-hover:text-electric transition-colors"
                  >Voir →</span
                >
              </div>
            </a>
