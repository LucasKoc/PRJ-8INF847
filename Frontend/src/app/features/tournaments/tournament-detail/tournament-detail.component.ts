      ...
      @if (loading()) {
        ...
        <div class="surface p-6 mb-4">
          <div class="flex items-start justify-between gap-4 mb-5 flex-wrap">
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2.5 mb-2 flex-wrap">
                ...
                <span class="label px-2 py-1 rounded inline-flex items-center gap-1.5" [class]="statusPill(tournament()!.status)">
                  <span class="w-1.5 h-1.5 rounded-full" [class]="dotClass(tournament()!.status)"></span>
                  {{ statusLabel(tournament()!.status) }}
                </span>
              </div>
              <p class="text-sm text-muted">
                {{ tournament()!.game }} · <span class="tag-mono text-ink">{{ tournament()!.format }}</span>
                · Organisé par <span class="text-ink">{{ tournament()!.organizer?.username ?? '—' }}</span>
              </p>
            </div>
            ...
            @if (isOrganizer() && allowedNext().length > 0) {
              <div class="flex gap-1.5 flex-wrap">
                @for (next of allowedNext(); track next) {
                  <button type="button" (click)="changeStatus(next)" class="btn btn-secondary btn-sm">
                    → {{ statusLabel(next) }}
                  </button>
                }
              </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-border">
            <div>
              ...
              <p class="text-sm text-ink tabular-nums">{{ tournament()!.startsAt | date: 'dd MMM yyyy' : '' : 'fr-FR' }}</p>
              <p class="text-xs text-muted tabular-nums">{{ tournament()!.startsAt | date: 'HH:mm' : '' : 'fr-FR' }}</p>
            </div>
            <div>
              <div class="label text-muted mb-1">Fin inscriptions</div>
              <p class="text-sm text-ink tabular-nums">{{ tournament()!.registrationDeadline | date: 'dd MMM yyyy' : '' : 'fr-FR' }}</p>
              <p class="text-xs text-muted tabular-nums">{{ tournament()!.registrationDeadline | date: 'HH:mm' : '' : 'fr-FR' }}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p class="display-xl text-2xl text-ink tabular-nums">
                {{ approvedCount() }}<span class="text-muted text-base font-normal">/{{ tournament()!.maxTeams }}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="surface p-6 mb-4 relative overflow-hidden">
          ...
          <div class="flex items-start gap-4">
            <div class="flex items-center justify-center w-10 h-10 rounded-md bg-electric/10 border border-electric/30 text-electric shrink-0">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-5 h-5">
                <path d="M6 3v18M18 3v18M3 6h3M3 12h3M3 18h3M18 6h3M18 12h3M18 18h3M6 8h12M6 16h12"/>
              </svg>
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1 flex-wrap">
                <h3>Bracket du tournoi</h3>
                <span class="label text-electric px-1.5 py-0.5 rounded bg-electric/10 border border-electric/30">À VENIR · V2</span>
              </div>
              <p class="text-sm text-muted leading-relaxed">
                Disponible à la clôture des inscriptions. La génération automatique du bracket (simple élimination avec byes) et l'affichage en arbre visuel arriveront dans une version ultérieure.
              </p>
            </div>
          </div>
        </div>
        ...
        @if (auth.isPlayer() && tournament()!.status === OPEN && captainTeams().length > 0) {
          <div class="surface p-6 mb-4">
            ...
            <form [formGroup]="regForm" (ngSubmit)="register()" class="flex gap-3 items-end flex-wrap">
              ...
              <button type="submit" [disabled]="regForm.invalid || registering()" class="btn btn-primary btn-md">
        ...
        <div class="surface p-6">
          <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
            <h2>Inscriptions</h2>
            <span class="text-xs text-muted tabular-nums">{{ registrations().length }} au total</span>
          </div>
                ...
                <li class="px-6 py-3.5 flex items-center justify-between gap-3 flex-wrap">
                  <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2 flex-wrap mb-0.5">
                      <span class="font-medium text-ink">{{ r.team?.name ?? 'Équipe #' + r.teamId }}</span>
                      @if (r.team?.tag) {
                        <span class="tag-mono text-xs bg-raised border border-border px-1.5 py-0.5 rounded">{{ r.team?.tag }}</span>
                      }
                  ...
                  <div class="flex items-center gap-1.5 flex-wrap">
                    @if (isOrganizer() && r.status === PENDING) {
                      <button type="button" class="btn btn-sm" style="background:var(--color-go);color:var(--color-abyss);" (click)="review(r.id, APPROVED)">
                        Approuver
                      </button>
                      <button type="button" class="btn btn-danger btn-sm" (click)="review(r.id, REJECTED)">
                        Rejeter
                      </button>
                    }
                    @if (auth.isPlayer() && isMyRegistration(r) && (r.status === PENDING || r.status === APPROVED)) {
                      <button type="button" class="btn btn-ghost btn-sm text-alert hover:text-alert hover:bg-alert/10" (click)="cancel(r.id)">
                        Annuler
                      </button>
                    }
                  </div>
...
export class TournamentDetailComponent implements OnInit {
  ...
  readonly captainTeams = computed(() => {
    ...
    const registeredIds = new Set(
        this.registrations()
            .filter((r) => r.status === RegistrationStatus.PENDING || r.status === RegistrationStatus.APPROVED)
  ...
  statusPill(s: TournamentStatus): string  { return STATUS_PILL[s]; }
  regStatusLabel(s: RegistrationStatus): string { return REG_STATUS_LABEL_FR[s]; }
  regPill(s: RegistrationStatus): string { return REG_PILL[s]; }
  ...
  review(regId: string, decision: RegistrationStatus.APPROVED | RegistrationStatus.REJECTED): void {
    const note = decision === RegistrationStatus.REJECTED
    ...
    this.registrationsService.review(regId, { status: decision, reviewNote: note }).subscribe({
      next: () => {
        this.toast.success(decision === RegistrationStatus.APPROVED ? 'Inscription approuvée' : 'Inscription rejetée');
        this.reloadRegistrations();
      },
      error: (err) => this.toast.error(this.parseErr(err)),
    });
