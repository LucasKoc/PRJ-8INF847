  ...
  template: `
    <div class="min-h-dvh flex flex-col">
        ...
        <div class="mx-auto max-w-7xl px-4 h-14 flex items-center gap-6">
          ...
          <div class="flex items-center gap-2 shrink-0">
            @if (auth.isAuthenticated()) {
              <div
                class="hidden sm:flex items-center gap-2 pl-2 pr-3 py-1 rounded-md bg-surface border border-border"
              >
                <span
                ...
                <span class="label text-muted pl-1 border-l border-border">{{
                  auth.user()?.role
                }}</span>
              </div>
      ...
      <footer class="border-t border-border py-6 mt-8">
        <div
          class="mx-auto max-w-7xl px-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-muted"
        >
...
export class MainLayoutComponent {
  ...
  readonly initial = computed(() => (this.auth.user()?.username ?? '?').charAt(0).toUpperCase());
